program main
  ...
  integer, parameter :: ldim_sq = ldim**2
  ...
  do iiter = 1, niter
    ...
    !$acc parallel vector_length(ldim_sq)
    !$acc loop gang
    do ibatch = 1, nbatch
      !$acc loop vector collapse(2)
        do j = 1, ldim
          do i = 1, ldim
            ...
            !$acc loop seq
            do k = 1, ldim
              Ctemp = Ctemp + Abatch(i,k,ibatch) * Bbatch(k,j,ibatch)
            enddo
            Cbatch(i,j,ibatch) = Cbatch(i,j,ibatch) + Ctemp
          enddo
        enddo
    enddo ! ibatch
    !$acc end parallel
